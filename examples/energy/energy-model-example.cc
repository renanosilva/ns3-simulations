 ...
 * every 1 second. Each node is powered by a BasicEnergySource, and the WiFi 
 * radio consumes energy for the transmission/reception of the packets.
 *
 * For the receiver node, the example prints the energy consumption of the WiFi radio,
 * and the residual energy in the energy source.
 *
 * The nodes initial energy is set to 0.1 J, the transmission and reception entail a
 * current consumption of 0.0174 A and 0.0197 A, respectively (default values in
 * WifiRadioEnergyModel). The energy source voltage is 3 V (default value in BasicEnergySource) 
 * and the residual energy level is updated every 1 second (default value).
 *
...
main(int argc, char* argv[])
{
    ...
    double Prss = -80;          // dBm (força do sinal - valor fraco)
    ...
    // The below set of helpers will help us to put together the wifi NICs (Network Interface Card) we want
    WifiHelper wifi;
    if (verbose)
    {
    ...
    // configuring energy source
    basicSourceHelper.Set("BasicEnergySourceInitialEnergyJ", DoubleValue(0.1));
    // installing energy source on nodes
    EnergySourceContainer sources = basicSourceHelper.Install(c);
    // creating device energy comsumption model
    // A DeviceEnergyModel is the energy consumption model of a device installed on the node.
    WifiRadioEnergyModelHelper radioEnergyHelper;
    // configuring radio energy model
    radioEnergyHelper.Set("TxCurrentA", DoubleValue(0.0174));
    //Faz a associação entre as placas de rede, suas fontes de energia e seu modelo de consumo de energia
