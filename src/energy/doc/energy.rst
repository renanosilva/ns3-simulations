...
* In the ``GenericBatteryModel`` charging behavior (voltage as a function of SoC - State of Charge) is included but is not been thoroughly tested. Testing requires the implementation of a harvesting device (A charger) capable of providing a CCCV charging method typically used in batteries.
