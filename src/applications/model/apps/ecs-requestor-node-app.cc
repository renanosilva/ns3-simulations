...
#include "ecs-requestor-node-app.h"
...
NS_LOG_COMPONENT_DEFINE("ECSRequestorNodeApp");
NS_OBJECT_ENSURE_REGISTERED(ECSRequestorNodeApp);

TypeId
ECSRequestorNodeApp::GetTypeId()
{
    NS_LOG_FUNCTION("ECSRequestorNodeApp::GetTypeId()");

    static TypeId tid =
        TypeId("ns3::ECSRequestorNodeApp")
            .SetParent<CheckpointApp>()
            .SetGroupName("Applications")
            .AddConstructor<ECSRequestorNodeApp>()
            .AddAttribute("Port",
                          "Port on which we listen for incoming packets.",
                          UintegerValue(9),
                          MakeUintegerAccessor(&ECSRequestorNodeApp::m_port),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute(
                "MaxPackets",
                "The maximum number of packets the application will send (zero means infinite)",
                UintegerValue(100),
                MakeUintegerAccessor(&ECSRequestorNodeApp::m_count),
                MakeUintegerChecker<uint32_t>())
            .AddAttribute("Interval",
                            "The time to wait between packets",
                            TimeValue(Seconds(1.0)),
                            MakeTimeAccessor(&ECSRequestorNodeApp::m_interval),
                            MakeTimeChecker())
            .AddAttribute("RemoteAddresses",
                            "List of destination addresses in 'IP;IP;IP...' format",
                            StringValue(""), // Valor padrão: vazio
                            MakeStringAccessor(&ECSRequestorNodeApp::GetPeerAddresses, &ECSRequestorNodeApp::SetPeerAddresses),
                            MakeStringChecker()) // Verifica se é uma string válida
            .AddAttribute("RemotePort",
                            "The destination port of the outbound packets",
                            UintegerValue(0),
                            MakeUintegerAccessor(&ECSRequestorNodeApp::m_peerPort),
                            MakeUintegerChecker<uint16_t>())
            .AddAttribute("ECSAddress",
                            "ECS's Address",
                            StringValue(""), // Valor padrão: vazio
                            MakeStringAccessor(&ECSRequestorNodeApp::GetECSAddress, &ECSRequestorNodeApp::SetECSAddress),
                            MakeStringChecker()) // Verifica se é uma string válida
            .AddAttribute("ECSPort",
                            "The destination port of the ECS",
                            UintegerValue(0),
                            MakeUintegerAccessor(&ECSRequestorNodeApp::ecsPort),
                            MakeUintegerChecker<uint16_t>())
            .AddAttribute("NodeName",
                            "This node's name. ",
                            StringValue(""),
                            MakeStringAccessor(&ECSRequestorNodeApp::nodeName),
                            MakeStringChecker())
            .AddAttribute("TotalNodesQuantity",
                            "Quantidade total de nós do sistema. ",
                            UintegerValue(0),
                            MakeIntegerAccessor(&ECSRequestorNodeApp::totalNodesQuantity),
                            MakeIntegerChecker<int>())
            .AddAttribute("ConfigFilename",
                            "This node's config filename. ",
                            StringValue(""),
                            MakeStringAccessor(&ECSRequestorNodeApp::configFilename),
                            MakeStringChecker());
...
ECSRequestorNodeApp::ECSRequestorNodeApp()
{
...
void
ECSRequestorNodeApp::StartApplication() {
    NS_LOG_FUNCTION(this);
    ...
    udpHelper->setReceiveCallback(MakeCallback(&ECSRequestorNodeApp::HandleRead, this));

    //Agendando envio de mensagens
    m_sendEvent = Simulator::Schedule(Seconds(0.1), &ECSRequestorNodeApp::Send, this);
...
void
ECSRequestorNodeApp::StopApplication()
{
    NS_LOG_FUNCTION(this);
...
ECSRequestorNodeApp::Send()
{
    ...
    try {
        ...
        for (Ipv4Address ip : m_peerAddresses){
            sendViaECS(REQUEST_VALUE, 0, ip, m_peerPort);
        }
...
void ECSRequestorNodeApp::HandleRead(Ptr<MessageData> md){
...
void ECSRequestorNodeApp::replayReceive(Ptr<MessageData> md, bool replayResponse, bool resendResponse){
    if (isSleeping() || isDepleted()){
        //ignora mensagens se estiver em modo sleep ou descarregado
        return;
    }
...
void ECSRequestorNodeApp::resetNodeData() {
    NS_LOG_FUNCTION(this);
...
void ECSRequestorNodeApp::scheduleSendEvent(){
    if (udpHelper->getSentMessagesCounter() < m_count || m_count == 0){
        //Agendando próximo envio
        m_sendEvent = Simulator::Schedule(m_interval, &ECSRequestorNodeApp::Send, this);
    }
}

uint64_t ECSRequestorNodeApp::GetTotalTx() const
{
    NS_LOG_FUNCTION(this);
    return udpHelper->getTotalBytesSent();
}

void ECSRequestorNodeApp::printNodeData(){
    NS_LOG_FUNCTION(this);
...
void ECSRequestorNodeApp::SetPeerAddresses(string addressList)
{
    NS_LOG_FUNCTION(this);
...
string ECSRequestorNodeApp::GetPeerAddresses() const {
    NS_LOG_FUNCTION(this);
...
uint16_t ECSRequestorNodeApp::getPeerPort() {
    return m_peerPort;
}

json ECSRequestorNodeApp::to_json() const {
    NS_LOG_FUNCTION(this);
...
void ECSRequestorNodeApp::from_json(const json& j) {
    NS_LOG_FUNCTION(this);
