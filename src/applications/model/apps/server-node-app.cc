...
void ServerNodeApp::replayReceive(Ptr<MessageData> md, bool replayResponse, bool resendResponse) {
    if (isSleeping() || isDepleted()){
        //ignora mensagens se estiver em modo sleep ou descarregado
        return;
    }
    ...
    if (md->GetCommand().find(REQUEST_VALUE) != string::npos){
        ...
        if (replayResponse){
            ...
            Ptr<MessageData> mdResp = send(REPLAY_RESPONSE_VALUE, m_seq, md->GetFrom(), !resendResponse);
        }
    }
