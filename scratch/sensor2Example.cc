...
#include "ns3/sensor-energy-model.h"
...
NS_LOG_COMPONENT_DEFINE("Sensor2Example");
...
/* Utilizado para definir se o nó pode ou não receber pacotes no momento, baseado nas regras
de requisições por segundo que podem ser atendidas (máximo de 2 requisições por segundo) */
static double segundoUltimaReqAtendida = -1;
...
ReceivePacket(Ptr<Socket> socket)
{
    ...
    while ((packet = socket->RecvFrom(from)))
    {
        if (packet->GetSize() > 0)
        {
            ...
            //Atende requisições a cada 0.5 segundos
            if (segundoAtual >= segundoUltimaReqAtendida + 0.5){
                //Atendendo a requisição
                NS_LOG_UNCOND("Requisição aceita");

                //ao receber uma requisição, o sensor deve efetuar uma leitura
                readSensorValue();

                //após a leitura, o sensor responde à requisição
                respondRequest(socket, packet, from);

                segundoUltimaReqAtendida = segundoAtual;
...
/**
 * Trace function for total wifi energy consumption at node.
 *
 * \param oldValue Old value
