...
static const string CONFIG_FILENAME = "scratch/07-no-rollback-2-with-ECS.json"; 
...
main(int argc, char* argv[])
{
    ...
    LogComponentEnable("ECSNodeApp", LOG_INFO);
    LogComponentEnable("ECSCheckpointApp", LOG_INFO);
    LogComponentEnable("ECSResponsorNodeApp", LOG_INFO);
    LogComponentEnable("ECSRequestorNodeApp", LOG_INFO);
    LogComponentEnable("Battery", LOG_INFO);
    LogComponentEnable("MessageData", LOG_INFO);
    ...
    LogComponentEnable("NoRollback2WithECS", LOG_INFO);
    LogComponentEnable("ECSProtocol", LOG_INFO);
    ...
    // LogComponentEnable("Application", LOG_ALL);
    // LogComponentEnable("CheckpointApp", LOG_ALL);
    // LogComponentEnable("ServerNodeApp", LOG_ALL);
    // LogComponentEnable("ClientApp", LOG_ALL);
    // LogComponentEnable("ECSNodeApp", LOG_ALL);
    // LogComponentEnable("ECSCheckpointApp", LOG_ALL);
    // LogComponentEnable("ECSResponsorNodeApp", LOG_ALL);
    // LogComponentEnable("ECSRequestorNodeApp", LOG_ALL);
    // LogComponentEnable("Battery", LOG_ALL);
    // LogComponentEnable("MessageData", LOG_ALL);
    // LogComponentEnable("CheckpointStrategy", LOG_ALL);
    ...
    // LogComponentEnable("EfficientAssyncRecoveryProtocol", LOG_ALL);
    // LogComponentEnable("EARPWithoutRollback", LOG_ALL);
    // LogComponentEnable("EARPWithoutRollbackV2", LOG_ALL);
    // LogComponentEnable("EARPWithoutRollbackV2Optimized", LOG_ALL);
    // LogComponentEnable("NoRollback2WithECS", LOG_ALL);
    // LogComponentEnable("ECSProtocol", LOG_ALL);
    // LogComponentEnable("EnergyGenerator", LOG_ALL);
    // LogComponentEnable("CircularEnergyGenerator", LOG_ALL);
    // LogComponentEnable("UDPHelper", LOG_ALL);
    // LogComponentEnable("LogUtils", LOG_ALL);
    ...
    vector<string> ecsNodesNames = config.GetNodesWithRole("ECS");
    vector<string> serverNodesNames = config.GetNodesWithRole("responsor");
    vector<string> clientNodesNames = config.GetNodesWithRole("requestor");
    int ecsNodesQuantity = 1; //a princípio, só poderá haver uma ECS
    int serverNodesQuantity = serverNodesNames.size();
    int clientNodesQuantity = clientNodesNames.size();
    int totalNodesQuantity = ecsNodesQuantity + serverNodesQuantity + clientNodesQuantity;
    ...
    NodeContainer ecsNode;
    NodeContainer clientNodes;
    NodeContainer serverNodes;

    //O primeiro nó registrado será a ECS
    for (int i = 0; i < ecsNodesQuantity; i++){
        ecsNode.Add(nodes.Get(i));
    }

    //Em seguida, os servidores
    for (int i = ecsNodesQuantity; i < (ecsNodesQuantity + serverNodesQuantity); i++){
        serverNodes.Add(nodes.Get(i));
    }

    //Os demais serão os clientes
    for (int i = (ecsNodesQuantity + serverNodesQuantity); i < totalNodesQuantity; i++){
    ...
    Ipv4Address ecsAddress;
    uint16_t ecsPort = 9000;
    vector<Ipv4Address> serverAddresses;
    uint16_t serverPort = 9;

    //Iniciando ECS
    //Cria uma aplicação de ECS, de forma a centralizar o tráfego de dados. 9 é a porta.
    ECSNodeAppHelper ecsAppHelper(ecsPort, ecsNodesNames[0], CONFIG_FILENAME, totalNodesQuantity);
    
    //Instala a aplicação no nó i e agenda sua inicialização
    ecsAppHelper.Install(ecsNode.Get(0)).Start(Seconds(1.0));
    ecsAddress = i.GetAddress(0);

    //Iniciando servidores
    for (int j = 0; j < serverNodesQuantity; j++){
        
        //Cria uma aplicação de servidor, de forma a gerar tráfego de dados. 9 é a porta.
        ResponsorNodeAppHelper serverAppHelper(serverPort, serverNodesNames[j], ecsAddress, ecsPort, CONFIG_FILENAME, totalNodesQuantity);
        
        //Instala a aplicação no nó j e agenda sua inicialização
        serverAppHelper.Install(serverNodes.Get(j)).Start(Seconds(1.0));
        serverAddresses.push_back(i.GetAddress(j + ecsNodesQuantity));
    }
    ...
    for (int j = 0; j < clientNodesQuantity; j++){
        ...
        RequestorNodeAppHelper clientAppHelper(serverAddresses, serverPort, ecsAddress, ecsPort, 
                                                clientNodesNames[j], CONFIG_FILENAME, totalNodesQuantity);
